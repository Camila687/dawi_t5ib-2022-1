...
public class FrmManteProd extends JFrame {
	...
	void registrar() {
		...
		List<Categoria> categorias = em.createQuery("select a from Categoria a Where a.descripcion=:xcategoria", Categoria.class).setParameter("xcategoria", cboCategorias.getSelectedItem()).getResultList();
		List<Proveedor> proveedor = em.createQuery("select p from Proveedor p Where p.nombre_rs=:xnombre", Proveedor.class).setParameter("xnombre", cboProveedores.getSelectedItem()).getResultList();
		...
		p.setIdcategoria(categorias.get(0).getIdcategoria());
		p.setIdprovedor(proveedor.get(0).getIdprovedor());
		p.setStock(stock);
		p.setPrecio(precio);
		p.setEstado(estado);
